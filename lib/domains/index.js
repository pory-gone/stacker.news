...
export const domainsMappingsCache = cachedFetcher(async function fetchDomainsMappings () {
  try {
          ...
          select: {
            ...
            branding: {
              select: {
                ...
                tagline: true,
                faviconId: true,
                // used as the manifest URL cache buster in _document.js
                updatedAt: true
              }
            }
          }
    ...
    return domains.reduce((acc, domain) => {
      const branding = domain.sub?.branding ?? null
      acc[domain.domainName.toLowerCase()] = {
        id: domain.id,
        domainName: domain.domainName,
        subName: domain.subName,
        subDesc: domain.sub?.desc ?? null,
        // normalize Prisma Date to ISO string to make it serializable
        subBranding: branding && {
          ...branding,
          updatedAt: branding.updatedAt?.toISOString?.() ?? branding.updatedAt ?? null
        },
 ...
 * with this shape: { id, domainName, subName, subDesc, subBranding, tokenVersion } */
...
export const getDomainBranding = async (domain) => {
  ...
  return {
    ...
    // lets the client build capture URLs for this domain (see lib/capture.js)
    domainName: domainMapping.domainName,
    ...domainMapping.subBranding,
